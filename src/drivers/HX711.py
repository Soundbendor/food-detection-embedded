...
Abstraction layer for the HX711 to allow us to add stablitiy improvements if needed
"""
...
"""
HX711 Driver Class Developed by Tatobari
https://github.com/tatobari/hx711py
"""
class HX711_Tatobari:
...
class HX711(DriverBase):

    """
    Constructo for the HX711

    :param refrence_unit: This is the number calculted from the calibration method to provide more accurate readings
    :param DT: The GPIO pin that data is connected to (default 5)
    :param SCK: The GPIO pin that clock is connected to (default 6)
    :param gain: setting for the sensor (default 128)
    :param channel: Which channel we are reading from (default A)
    """
    def __init__(self, refrence_unit=407.67777777777775, DT=5, SCK=6, gain=128, channel='A'):
        super().__init__("HX711")
        self.hx711 = HX711_Tatobari(dout=DT, pd_sck=SCK, gain=gain)

        # Set the data fromat and reference unit
        self.hx711.set_reading_format("MSB", "MSB")
        self.hx711.set_reference_unit(refrence_unit)
        self.channel = channel
    
    """
    Initialize the sensor, in this case we just reset it which initiates a power cycle
    Handle any exceptions and print out the exception
    """
    def initialize(self):
        # Power down and power the device back on
        self.hx711.reset()
        self.hx711.tare()
        logging.info("Resetting and Taring HX711...")
        return True
       
    
    """
    Request a measurement from the sensor and store it locally in the class
    """
    def measure(self):
        readings = []
        if(self.channel == 'A'):
            for i in range(3):
                readings.append(self.hx711.get_weight_A())
        else:
            for i in range(3):
    ...
    """
    Tare whichever channel we are using
    """
    def tareScale(self):
        if(self.channel == 'A'):
            self.hx711.tare_A()
    ...
    """
    Run an automatic calibration routine to calculate the reference unit
    """
    def calibrate(self):
        ...
        logging.info(f"Calculated refrence unit: {refrenceUnit}")
        self.hx711.set_reference_unit(refrenceUnit)
        logging.info("HX711 has been calibrated!")
